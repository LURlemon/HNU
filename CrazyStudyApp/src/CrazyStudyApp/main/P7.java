...
//		icon=new ImageIcon(getClass().getResource("/img/pic7.jpg"));
//		img=icon.getImage();

		JLabel oldPasswordLable = new JLabel("请输入原密码");
		oldPasswordLable.setBounds(230, 80, 120, 25);
		oldPasswordLable.setFont(new Font("方正准圆_GDK", Font.PLAIN, 14));
		this.add(oldPasswordLable);

		JPasswordField oldPasswordText = new JPasswordField(20);
		oldPasswordText.setBounds(330, 80, 200, 25);
		oldPasswordText.setEchoChar('\0');
		oldPasswordText.setText("请输入原密码");
		oldPasswordText.setForeground(Color.LIGHT_GRAY);
		oldPasswordText.addFocusListener(new FocusListener()
		{
			@Override
			public void focusLost(FocusEvent e)
			{// 失去焦点的时候
				String pswd = new String(oldPasswordText.getPassword()).trim();
				if (pswd.equals(""))
				{// 如果没有输入密码. 就用明文 提示用户输入密码
					oldPasswordText.setEchoChar('\0');
					oldPasswordText.setText("请输入原密码");
					oldPasswordText.setFont(new Font("方正准圆_GDK", Font.PLAIN, 12));
					oldPasswordText.setForeground(Color.red);
				}
			}
			...
			{// 获得焦点的时候
				String pswd = new String(oldPasswordText.getPassword()).trim();
				if (pswd.equals("请输入原密码"))
				{// 如果是密码提示文字 ,那么就清空文字, 并设置密文显示
					oldPasswordText.setText("");
					oldPasswordText.setEchoChar('*');
					oldPasswordText.setForeground(Color.BLACK);
				}
			}
		});
		this.add(oldPasswordText);

		JLabel firstPasswordLable = new JLabel("请输入新密码");
		firstPasswordLable.setBounds(230, 140, 120, 25);
		...
		firstPasswordText.setBounds(330, 140, 200, 25);
		firstPasswordText.setEchoChar('\0');
		firstPasswordText.setText("请输入新密码");
		...
		tipLable.setBounds(390, 170, 150, 25);
		tipLable.setFont(new Font("方正准圆_GDK", Font.PLAIN, 8));
		this.add(tipLable);

		JLabel tipLableT = new JLabel("");
		tipLableT.setBounds(470, 230, 150, 25);
		tipLableT.setFont(new Font("方正准圆_GDK", Font.PLAIN, 8));
		this.add(tipLableT);

		JLabel tipLableO = new JLabel("");
		tipLableO.setBounds(470, 110, 150, 25);
		tipLableO.setFont(new Font("方正准圆_GDK", Font.PLAIN, 8));
		this.add(tipLableO);

		JLabel secondPasswordLable = new JLabel("请确认密码");
		secondPasswordLable.setBounds(230, 200, 80, 25);
		...
		secondPasswordText.setBounds(330, 200, 200, 25);
		secondPasswordText.setEchoChar('\0');
		secondPasswordText.setText("请确认新密码");
		...
		confirmButtonTwo.setBounds(340, 260, 130, 40);
		...
		confirmButtonTwo.addMouseListener(new MouseAdapter()
		{
			public void mouseClicked(MouseEvent e)
			{
				if (e.getButton() == MouseEvent.BUTTON1)
				{
					...
					if (!SolveAccountClass.loginMap.get(Main.phoneNumber).equals(oldPasswordText.getText()))
					{
						tipLableO.setText("原密码不正确！");
						tipLableO.setForeground(Color.red);
						tipLable.setText("");
						tipLableT.setText("");
					}
